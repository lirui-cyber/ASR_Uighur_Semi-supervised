...
int main1(std::string save_path,std::string file_name,std::vector<float> start,std::vector<float> end) {
    ...
    if(start.size()==end.size()){
        for(int i =0;i<start.size();i++){
            ...
            if(pre_start<start_time){
                std::stringstream ss_pre;
                ss_pre << save_path<<"_"<<pre_start<<"_"<<end_time<< ".wav";
                // 从字符串流中获取结果字符串
                std::string result_pre = ss_pre.str();
                std::string output_file_pre = result_pre;
                split_audio(input_file, output_file_pre, pre_start, end_time);
                pre_start = end_time;
            }
              else{
