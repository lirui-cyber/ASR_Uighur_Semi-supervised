...
private:
    ...
    void init_onnx_model(const std::string& model_path)
    {
        ...
        std::wstring path_tmp = std::wstring(model_path.begin(), model_path.end());
...
public:
    // Construction  pre_min 64 Threshold 0.5
    VadIterator(const std::string ModelPath,
        int Sample_rate = 16000, int windows_frame_size = 64,
        float Threshold = 0.6, int min_silence_duration_ms = 0,
        int speech_pad_ms = 64, int min_speech_duration_ms = 2000,
        float max_speech_duration_s = 13000)
    {
...
int main(int argc ,char *argv[])
{
    ...
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <input_model> <input_file> <output_dir>" << std::endl;
        return 1;
    }
    ...
    // Read wav "/home/zwb521/test_wav/看了又看第四部05_维.wav"
    wav::WavReader wav_reader(argv[2]); //16000,1,32float
    ...
    // ===== Test configs ===== 
    //"/home/zwb521/ur_kws_server/server/bulid_server/vad_onnx/vad_model/silero_vad.onnx"
    ...
    std::vector<float> end_data;
    stamps = vad.get_speech_timestamps();
    for (int i = 0; i < stamps.size(); i++) {

        // std::cout << stamps[i].c_str() << std::endl;
        // std::cout<<stamps[i].start<<std::endl;
        // std::cout<<stamps[i].end<<std::endl;
        start_data.push_back(stamps[i].start/16000.0);
        end_data.push_back(stamps[i].end/16000.0);
        // std::cout<<start_data[i]<<std::endl;
        // std::cout<<end_data[i]<<std::endl;
    }



    //这样处理目录 linux环境可能会有问题
    std::filesystem::path pathObj(argv[2]); //argv[2] 是文件名
    std::string fileName = pathObj.stem().string();
    std::string fileName_txt = fileName;
    std::string fileName_dir = fileName;

    std::filesystem::path currentDir = std::filesystem::current_path();
    // std::filesystem::path vad_save_file = currentDir/ "vad_log"/fileName_txt.append("vad.txt");
    // writeVectorToFile(start_data, end_data, vad_save_file.string());

    std::filesystem::path part_dir = argv[3]; // argv[3]输出文件目录 

    std::filesystem::path splitDir = part_dir/fileName_dir.append("_split");
    std::filesystem::create_directory(splitDir);

    std::cout << "fileName:" << fileName << std::endl;
    std::filesystem::path splitsaveDir = splitDir / fileName;

    main1(splitsaveDir.string(),argv[2],start_data,end_data); //save split audio
    //分割完毕 容器清空
    start_data.clear();
    end_data.clear();
    return 0;
